---
hide:
  - navigation
  - toc
---

# Tej Singh Rajpurohit

### Healthcare Business Analyst  
**Healthcare Technology · AI Product · Pharma Domain**

Healthcare domain professional with a **Master of Pharmacy in Pharmacology** and real-world experience in **Business Development** and **Regulatory Affairs**.

Transitioning into **Healthcare Business Analysis and Healthcare Technology**—combining pharmaceutical domain knowledge with Business Analysis, Agile, product thinking, data, and technical skills.

I build structured requirements and solution documentation across the healthcare product lifecycle: from business problem and stakeholder analysis through process models, BRD/FRD, user stories, backlog, UAT, and traceability.

[View Resume (PDF)](assets/Tej-Singh-Rajpurohit-Resume.pdf){ .md-button .md-button--primary }
[LinkedIn](https://www.linkedin.com/in/tej-singh-rajpurohit-39071694/){ .md-button }
[GitHub Repository](https://github.com/tsrajpurohit/healthcare-ba-ai-portfolio-){ .md-button }
[How I approach BA](ba-approach.md){ .md-button }

---

### Target roles

**Healthcare Business Analyst · Business Systems Analyst · Healthcare Product Analyst · AI Business Analyst**

---

!!! note "Portfolio transparency"
    The healthcare technology projects on this site are **simulated Business Analysis case studies** created for professional learning and portfolio demonstration. They are **not** commercial client engagements or employment projects.  
    My pharmaceutical **Business Development** and **Regulatory Affairs** experience below is **real professional experience**.

---

## Career focus

| Area | What I contribute |
|------|-------------------|
| **Business Analysis** | Requirements elicitation, BRD/FRD, stakeholder analysis, process modeling, user stories, UAT, RTM |
| **Healthcare Technology** | Healthcare workflows, clinical processes, RCM, hospital operations, healthcare analytics |
| **AI Product** | AI opportunity analysis, use-case prioritization, business rules, human-in-the-loop, explainability |
| **Agile / Product** | User stories, acceptance criteria, backlog, Jira, sprint support, product requirements |
| **Data & Integration** | SQL/MySQL, Power BI, REST API concepts, Postman |
| **UX / Process Design** | BPMN, UML, Figma, Whimsical, workflow and user-journey modeling |

---

## Featured projects

### Primary

#### [Hospital Management System](hms/index.md)
*Business Analysis case study — core BA fundamentals*

Healthcare workflow analysis covering registration, appointments, patient management, and billing.

**Artifacts:** Project Charter · Business Case · Stakeholders · BRD / FRD · AS-IS / TO-BE · User Stories · Acceptance Criteria · Jira Backlog · UAT · RTM · Risks  

[Explore Hospital Management System →](hms/index.md)

---

#### [AI Healthcare Revenue Cycle Management](rcm/index.md)
*AI product / BA case study — flagship (AI + healthcare + product)*

AI-enabled RCM concept for claims, denials, coding assistance, and intelligent workqueues—with human-in-the-loop controls.

**Focus:** AI opportunity analysis · use-case prioritization · business rules · BRD / FRD · backlog · UAT  

[Explore AI Healthcare RCM →](rcm/index.md)

---

#### [Clinical Trial Management System](ctms/index.md)
*Business Analysis case study — pharma-aligned domain*

Trial setup, sites, subjects, visits, documentation, and compliance-oriented workflows.

**Focus:** Clinical workflows · process modeling · functional & non-functional requirements · user stories · UAT · RTM  

[Explore Clinical Trial Management →](ctms/index.md)

---

### Supporting

#### [Healthcare Analytics Dashboard](analytics/index.md)
*BA / product case study — KPIs & reporting*

Operational and executive dashboard requirements, KPI dictionary, Power BI–oriented requirements, and validation.

[Explore Healthcare Analytics →](analytics/index.md)

---

#### [AI Pharmacovigilance — VigilAI](pv/index.md)
*AI + pharmacovigilance — specialization path*

Connects pharmacology background with case processing, literature monitoring, signal detection, AI requirements, and auditability.

[Explore VigilAI →](pv/index.md)

---

## What you will find in this portfolio

Each case study follows a structured BA approach and may include:

Project Charter · Business Case · Stakeholder Register · RACI · BRD · FRD · AS-IS / TO-BE · Gap Analysis · User Stories · Acceptance Criteria · Product Backlog · UAT · RTM · Risk analysis · AI opportunity analysis · Dashboard / product requirements

Prefer starting from each project’s **overview** (problem → role → deliverables → outcome), then open detailed artifacts only as needed.

---

## My Business Analysis approach

I translate a business problem into a structured, testable solution:

| Step | Focus |
|------|--------|
| **01 — Understand** | Business problem · stakeholders · objectives · context |
| **02 — Analyze** | Requirements · AS-IS · pain points · gaps · business rules |
| **03 — Design** | TO-BE process · BRD / FRD · journeys · functional requirements |
| **04 — Deliver** | User stories · acceptance criteria · Jira backlog · sprint support |
| **05 — Validate** | UAT · RTM · issue analysis · business validation |
| **06 — Improve** | Feedback · process improvement · requirement refinement |

Full lifecycle diagram and principles: **[How I approach a business problem](ba-approach.md)**

---

## Skills & tools

**Business Analysis**  
Requirements elicitation & analysis · Stakeholder management · BRD · FRD · NFR · Gap analysis · Root cause & impact analysis · UAT · RTM  

**Agile & product**  
Agile · Scrum · User stories · Acceptance criteria · Product backlog · Backlog refinement · Sprint planning · Jira  

**Process & modeling**  
BPMN · UML · AS-IS / TO-BE · Process mapping · Workflow design · User journeys · Wireframing  

**Technical & data**  
REST API concepts · Postman · SQL · MySQL · Power BI · Data analysis & visualization  

**Design & collaboration**  
Figma · Whimsical · Draw.io · GitHub · MkDocs  

---

## Professional experience

### Espee Drugs & Finechem Co.  
**Assistant Manager – Business Development** · *July 2019 – September 2020*

- Buyer and supplier requirement coordination  
- Technical issue resolution  
- New product development support  
- QA / QC coordination  
- Regulatory requirement coordination  
- Cross-functional stakeholder communication  

### Lyka Labs Limited  
**Junior Executive – Regulatory Affairs** · *November 2018 – June 2019*

- Regulatory documentation  
- COPP documentation  
- Product permissions  
- FDCA-related documentation  
- Pharmaceutical manufacturing documentation  
- Export-related regulatory requirements  

---

## Education

- **Master of Pharmacy — Pharmacology**  
  Lachoo Memorial College of Science & Technology, Rajasthan University of Health Sciences, Jaipur  
  *March 2014 – July 2015*

- **Bachelor of Pharmacy**  
  Rajasthan University of Health Sciences, Jaipur  
  *September 2010 – July 2013*

---

## Resume & profile

[Download Resume](assets/Tej-Singh-Rajpurohit-Resume.pdf){ .md-button .md-button--primary }
[Connect on LinkedIn](https://www.linkedin.com/in/tej-singh-rajpurohit-39071694/){ .md-button }
[View GitHub](https://github.com/tsrajpurohit/Healthcare-BA-Case-studies){ .md-button }
[About Me](about.md){ .md-button }
